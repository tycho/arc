...
int Net_CrissCross::ReceiveTCP ( int *_packetCount )
{
    ...
    ARCReleaseAssert ( _packetCount != NULL );
    *_packetCount = 0;
    ...
    while ( true )
    {
        ...
        if ( !bytesInBuffer )
        {
            ...
            ARCDebugAssert ( length == sizeof ( packetSize ) );
        ...
        ARCDebugAssert ( (short)length <= packetSize );
        bytesInBuffer += length;
        ...
        ARCDebugAssert ( bytesInBuffer - sizeof(packetSize) == packetSize );
...
int Net_CrissCross::Send ( const Packet *_buffer, bool _criticalData )
{
    ...
    ARCReleaseAssert ( dataSent == (int)serializedDataSize );
